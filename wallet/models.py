...
class WalletTransactions(models.Model):
    """Model representing transactions in the user's wallet."""
    ...
    def __str__(self):
        return f"{self.transaction_type.capitalize()} of {self.amount:.2f} on {self.created_at}"


class WithdrawingRequests(models.Model):
    """Model representing withdrawal requests made by users."""

    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE)
    amount = models.DecimalField(max_digits=12, decimal_places=2)
    status = models.CharField(
        max_length=10, default='pending', choices=[
            ('pending', 'Pending'), 
            ('success', 'Success'), 
            ('rejected', 'Rejected')
        ]
    ...
    payment_obj = models.ForeignKey(WalletTransactions, on_delete=models.CASCADE)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"Withdrawal Request for {self.user.username} of {self.amount:.2f}"
