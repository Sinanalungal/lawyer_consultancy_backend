...
class VerifiedLawyerProfileListView(generics.ListAPIView):
    """
    API view to retrieve a list of verified lawyer profiles.

    This view allows authenticated users to search for lawyers by
    various parameters such as name, department, language, and
    experience level.
    """
    ...
    def get_queryset(self):
        """
        Retrieve the queryset of verified lawyer profiles based on 
        the search parameters provided in the request.

        Returns:
            queryset: A queryset of verified lawyer profiles filtered 
        ...
        if experience and experience != "All":
            if experience == "Less than five":
                queryset = queryset.filter(experience__lt=5)
